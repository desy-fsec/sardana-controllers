...
{
	/**
	 * The create controller method for KohzuSCAxis controller.
	 */	
	Controller *_create_KohzuSCAxis(const char *,vector<Controller::Properties> &);
}

/**
 * @brief A  Motor controller for KohzuSCAxis
 */
class KohzuSCAxis:public MotorController
{
	
public:
	/// Constructor
	KohzuSCAxis(const char *,vector<Controller::Properties> &);
	/// Destructor
	virtual ~KohzuSCAxis();
...
protected:
	...
	std::map<int32_t, KohzuSCAxisData*> motor_data; 
