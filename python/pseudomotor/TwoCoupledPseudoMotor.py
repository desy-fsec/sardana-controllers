...
class TwoCoupledPseudoMotor(PseudoMotorController):
    ...
    ctrl_properties = {'tolerance' : {'Type':'PyTango.DevFloat', 
                                      'Description': ('Tolerance is the maximum'
                                                      'difference between motor'
                                                      'positions. If it is -1,'
        ...
        if self.tolerance != -1:
            ...
            for index, c_pos in enumerate(curr_physical_pos[1:]):
                ...
                if (diff > self.tolerance):
                    ...
                    msg=('Tolerance has been exceeded. You must change the %s '
                         'to %f' % (slave_name, pos_master))
                    raise Exception(msg)
