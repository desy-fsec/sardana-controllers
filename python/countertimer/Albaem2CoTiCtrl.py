...
from sardana.pool.controller import (CounterTimerController, Type, Access,
                                     Description, NotMemorized, Memorize, 
                                     Memorized)
import socket 
import time
...
TRIGGER_INPUTS ={'DIO_1': 0,'DIO_2': 1,'DIO_3': 2, 'DIO_4': 3, 'DIFF_IO_1': 4,
                 'DIFF_IO_2': 5, 'DIFF_IO_3': 6,'DIFF_IO_4': 7 ,'DIFF_IO_5': 8,
                 'DIFF_IO_6': 9, 'DIFF_IO_7': 10,'DIFF_IO_8': 11,'DIFF_IO_9': 12}
...
#                        }
    ...
    def StateAll(self):
        ...
        if state in ['STATE_ACQUIRING', 'STATE_RUNNING']: 
            ...
            self.state = State.Fault
            self._log.debug("StateAll(): %r %r UNKNWON STATE: %s" %(self.state, self.status), state)
        self.status = state
    ...
    def LoadOne(self, axis, value, repetitions):
        #self._log.debug("LoadOne(%d, %f, %d): Entering...", axis, value, repetitions)
        if axis != 1:
            raise Execption('The master channel should be the axis 1')
        ...
        if self._synchronization in [AcqSynch.SoftwareTrigger, AcqSynch.SoftwareGate]:
    ...
    def StartAllCT(self):
        ...
        if self._synchronization in [AcqSynch.SoftwareTrigger, AcqSynch.SoftwareGate]:
            ...
            # APPEND SWTRIG TO THE START COMMAND OR SEND ANOTHER COMMAND TRIG:SWSEt
            cmd += ' SWTRIG'
