...
class DCM_TurboPmacController(TurboPmacCtrl.TurboPmacController):
    ...
    MaxDevice = 2
    class_prop = {'VCMPitchName':{'Type' : 'PyTango.DevString', 'Description' : 'VCM pitch pseudomotor'}}
    class_prop.update(TurboPmacCtrl.TurboPmacController.class_prop)
    
    ctrl_extra_attributes = {"EnergyMotion":{ "Type":"PyTango.DevBoolean", "R/W Type": "PyTango.READ_WRITE"}}
    ctrl_extra_attributes.update(TurboPmacCtrl.TurboPmacController.ctrl_extra_attributes) 
    
    def __init__(self,inst,props):
        TurboPmacCtrl.TurboPmacController.__init__(self,inst,props)
        self.vcm_pitch  = PoolUtil().get_motor(self.inst_name, self.VCMPitchName)
        self.superklass = TurboPmacCtrl.TurboPmacController 
        ...
        if len(self.startMultiple) > 1:
            PyTango.Except.throw_exception("CLAESS_DCM_PmacController.StartAll()", 
                                           "Error trying to start motion, this controller doesn't allow to use motors by Sardana pseudomotors composed from more than one motor",
                                            "StartAll()")
        if self.attributes[1]["EnergyMotion"]:
