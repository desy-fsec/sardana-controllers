#ifndef _FMBOXFDCMMOTORMACTRL_H_
#define _FMBOXFDCMMOTORCTRL_H_
...
{
	/**
	 * The create controller method for FMBOxfDCMMotor controller.
	 */	
	Controller *_create_FMBOxfDCMMotor(const char *,vector<Controller::Properties> &);
}

/**
 * @brief A  Motor controller for FMBOxfDCMMotor
 */
class FMBOxfDCMMotor:public MotorController
{
	
public:
	/// Constructor
	FMBOxfDCMMotor(const char *,vector<Controller::Properties> &);
	/// Destructor
	virtual ~FMBOxfDCMMotor();
...
protected:
	...
	std::map<int32_t, FMBOxfDCMMotorData*> motor_data; 
	

	stringstream            convert_stream;
};

#endif /*_FMBOXFDCMMOTORCTRL_H_*/
