#ifndef _HEXAPODCTRL_H_
#define _HEXAPODCTRL_H_
...
{
	/**
	 * The create controller method for Hexapod controller.
	 */	
	Controller *_create_Hexapod(const char *,vector<Controller::Properties> &);
}

/**
 * @brief A  Motor controller for Hexapod
 */
class Hexapod:public MotorController
{
	
public:
	/// Constructor
	Hexapod(const char *,vector<Controller::Properties> &);
	/// Destructor
	virtual ~Hexapod();
...
protected:
	...
	Tango::DeviceProxy      *hexapod_proxy;
	...
	std::map<int32_t, HexapodData*> motor_data; 
	

    stringstream            convert_stream;
};

#endif /*_HEXAPODCTRL_H_*/
