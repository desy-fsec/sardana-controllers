...
{
	/**
	 * The create controller method for Lom controller.
	 */	
	Controller *_create_Lom(const char *,vector<Controller::Properties> &);
}

/**
 * @brief A  Motor controller for Lom
 */
class Lom:public MotorController
{
	
public:
	/// Constructor
	Lom(const char *,vector<Controller::Properties> &);
	/// Destructor
	virtual ~Lom();
...
protected:
	...
	std::map<int32_t, LomData*> motor_data; 
