#ifndef _HASYLABDAC_H_
#define _HASYLABDAC_H_
...
{
	/**
	 * The create controller method for HasylabDAC controller.
	 */	
	Controller *_create_HasylabDAC(const char *,vector<Controller::Properties> &);
}

/**
 * @brief A  Motor controller for HasylabDAC
 */
class HasylabDAC:public MotorController
{
	
public:
	/// Constructor
	HasylabDAC(const char *,vector<Controller::Properties> &);
	/// Destructor
	virtual ~HasylabDAC();
...
protected:
	...
	vector<double> 		wanted_dac_pos;
	vector<int32_t>		wanted_dac;

	struct HasylabDACData 
	{
	...
	std::map<int32_t, HasylabDACData*> dac_data; 


    stringstream            convert_stream;
};

#endif /*_HASYLABDAC_H_*/
