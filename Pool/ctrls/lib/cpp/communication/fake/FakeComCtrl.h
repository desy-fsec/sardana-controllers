#ifndef _FAKECOMCTRL_H_
#define _FAKECOMCTRL_H_
...
extern "C"
{
	 ...
	 */	
	Controller *_create_FakeComController(const char *,vector<Controller::Properties> &);
}

/**
 * @brief A Communication controller that fakes send and receive operations
 */
class FakeComController:public ComController
{
	double CppComCh_extra_2;	///< dummy property value. Means nothing. For test purposes
	
public:
	/// Constructor
	FakeComController(const char *,vector<Controller::Properties> &);
	/// Destructor
	virtual ~FakeComController();
